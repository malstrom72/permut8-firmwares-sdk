...
Then wrap the compiled firmware into a bank with the bank writer,
...
`createP8Bank.nuxjs.js` does not currently expose command-line options for setting different
...
- `CurrentProgram` in V2 and `Current Program` in V1 point at the selected slot; bundled
  banks use `A0`.
- Clean banks generated by `createP8Bank.nuxjs.js` omit `Name`; named factory and firmware banks
  include it.
...
`createP8Bank.nuxjs.js` does not yet generate varied programs itself. For now, generate a clean
...
```sh
references/permut8-firmwares-sdk/tools/bin/NuXJS \
  references/permut8-firmwares-sdk/tools/gazlCompactor.nuxjs.js \
  <path-to-compiled.gazl> \
...
4. Generate the `.p8bank` with `tools/bin/NuXJS tools/createP8Bank.nuxjs.js`.

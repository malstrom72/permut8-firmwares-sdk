...
DOCLING_BIN="${DOCLING_BIN:-$VENV_DIR/bin/docling}"
GUIDE_DIR="$ROOT_DIR/docs"
PDF_PATH="$GUIDE_DIR/Permut8 User Guide.pdf"
MD_PATH="$GUIDE_DIR/Permut8 User Guide.md"
JSON_PATH="$GUIDE_DIR/Permut8 User Guide.json"
DOCLING_JSON_PATH="$GUIDE_DIR/Permut8 User Guide.docling.json"

if [ ! -x "$DOCLING_BIN" ]; then
	echo "Docling is not installed. Run tools/bootstrap-docling.sh first, or set DOCLING_BIN." >&2
	exit 1
fi
